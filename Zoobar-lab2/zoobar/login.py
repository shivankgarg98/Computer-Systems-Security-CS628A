...
class User(object):
    ...
    def addRegistration(self, username, password):
        token = ac.register(username, password)
        if token is not None:
            return self.loginCookie(username, token)
        else:
            return None
    ...
    def checkCookie(self, cookie):
        ...
        if ac.check_token(username, token):
            self.setPerson(username, token)

    def setPerson(self, username, token):
        self.person = ac.get_person(username) 
        self.creds = ac.get_creds(username) 
       
        self.token = token
        self.zoobars = self.person.zoobars 
