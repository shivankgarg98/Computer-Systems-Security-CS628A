...
pid_t launch_svc(CONF *conf, const char *name)
{
    ...
    if ((dir = NCONF_get_string(conf, name, "dir")))
    {
      ...
      if (chdir(dir))
      {
        warnx("%s %s", dir, "is unchanged");
        err(1, "%s %s", dir, "is not accessible");
      }
      if (getuid() != 0)
      {
          err(1,"%s", "not enough permission");

      }

        if (chroot(dir))
        {
          err(1,"%s %s",dir, " is unable to changed to new root");
        }
        warnx("%s %s","new root dir =",  dir);
    }
    ...
    if (NCONF_get_number_e(conf, name, "gid", &gid))
    {
        ...
        warnx("%s %ld","changin setgid", gid); 
        if (setresgid(gid, gid, gid))
        {
          err(1, "%s", "error changing gid");
        }
        warnx("%s %ld","success gid : ",gid); 
    }
    ...
    if ((groups = NCONF_get_string(conf, name, "extra_gids")))
    {
        ...
        {
            err(1, "%s", "error: setgroups");
        }

        warnx("%s", "success : setgroups" );

        for (i = 0; i < ngids; i++)
        {

            warnx("%s %d","extra gids: ", gids[i]);
        }
    }
    if (NCONF_get_number_e(conf, name, "uid", &uid))
    {
        /* change real, effective, and saved uid to uid */
        warnx("%s %ld","changin", uid);
    
        if (setresuid(uid, uid, uid))
        {
          err(1,"%s", "error changing uid");
        }
        warnx("%s %ld","success uid : ",uid);
    }
